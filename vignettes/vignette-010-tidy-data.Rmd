...
### Data to ship with the package
#### Correlation Figure

All of the following data sources will be read in as raw data from `slfrsk/data/` (originally cleaned a bit initially from raw data version in `slfrsk/data-raw/`) and then cleaned up individually. Depending on the situation, the resultant "cleaned/tidied" data may be saved to `slfrsk/data/` as a new `.rda` file.

##### Maxent extractions (states and countries), we are going to summarize the mean of the models as a grand mean
  
```{r bring in maxent extraction summary data}
...
##### trade data (states and countries, both present and future)
###### states value
...
###### states mass (this also converts mass from kg to metric tons)
...
######countries mass (this also converts from kg to metric tons)
...
#####grape yield (metric tons/ha) and production (metric tons)
**NOTE:** The Philippines has such low grape production that the  $log_{10}$ transformation renders it negative but by adding one prior to transformation, it becomes positive. It will likely be removed from analyses downstream.
```{r read in and summarize both grape yield and production}
data("states_grapes")
...
#####wine production (metric tons) 
  
```{r read in and summarize wine production}
data("states_wine")
...
```{r make countries_summary_present}
#add in the binaries
...
######Countries Future
This also includes manually changing the **United States** to a grape producer.
```{r make countries_summary_future}
#add in the binaries
...
####Correlation Plots
```{r save correlation plot  data, eval=FALSE}
if(FALSE){
...
##NOTES NOTES NOTES
*TRY TO FIX THESE COUNTRY NAMES*
"S\x8bo Tom\x8e and Pr\x92ncipe"  
"Cura\x8dao" 
"Saint Barth\x8elemy"
"C\x99te d'Ivoire"

####Map Figure
  1. wineries list (usa_wineries)
  2. states_centers_wine
  3. states_centers
  4. enm_data_usa_mean <- raster("./data/geotiff_enms//slftoh_usa_downsampled_x4_mean.tif")
  5. enm_data_mean <- raster("./data/geotiff_enms/slftoh_downsampled_x4_mean.tif")
  6. trade data
  7. slf coords? slf_coords <- read_csv("./data/Spotted Lanternfly/MAXENT_INPUT_VARIATE/SLF_FINAL_withPDA_24Sept18_v4_8.csv") %>%
