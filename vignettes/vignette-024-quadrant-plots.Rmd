...
if(present_transport == FALSE){
  ...
  (states_plot <- ggplot(data = data_to_plot) +
      ...
      #new add in the means of the wine groupings
        if(plot_mean) {
          geom_point(data = data_to_plot %>% group_by(wine) %>% summarize(n = n(), sd_x = sd(x_to_plot), sd_y = sd(y_to_plot), x_to_plot_mean = mean(x_to_plot), y_to_plot_mean = mean(y_to_plot)) %>% mutate(se_x = sd_x/n, se_y = sd_y / n, fill_to_plot = wine), 
