...
To merge the new `market_size` dat with our existing `countries` data, we use `left_join()` to merge the `market` and `summary` data by `ID`. We then remove the countries from the dataset that already have *Lycorma*. Note: we use a modified version of `%in%` and `Negate()` to create `%notin%` for removing countries.
...
With the predicted **risk**, we were able plot the scaled `risk` and `market_size`, resulting in the published risk of *Lycorma* to shock the global wine market plot.
