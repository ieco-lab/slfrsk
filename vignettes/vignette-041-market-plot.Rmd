...
countries_risk_market <- countries_summary_future %>% 
  ...
  filter(ID %notin% c("United States", "China", "India", "Vietnam"))#%>% 
  #dplyr::select(ID, 
  #              market_size,
  #              log10_avg_prod, 
...
simulationOutput <- simulateResiduals(fittedModel = risk_mod, plot = T) 
...
market_scatter<- countries_risk_market %>%
  ...
  ggtitle(label = paste('Spotted Lanternfly Paninvasion Risk Severity =',shock_cor,'of 10')) +
  geom_point(pch = 21) + 
...
pdf(file.path(here::here(),
              "vignettes", 
              paste0("market_scatter_v2_7.pdf")),
    width = 8.5, height = 11)
