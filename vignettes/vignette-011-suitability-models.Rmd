...
write_csv(x = env.corr, path = file.path(here(),"data-raw/env_cor_v1_downsampled.csv"), col_names = TRUE)
...
[UNSURE OF WHAT TO REPORT HERE FOR SUMMARY]
...
To make visualization more tractable, we reduced the size of the SDMs by using `raster::aggregate(fact = 4, fun = mean, expand = TRUE, na.rm = TRUE)` to reduce the SDM raster resolution. We also clipped the SDMs to the continental US to produce the files used for separate `states` and `countries` visualization. The three models can be visualized below for the US and then the world:

```{r visualize models at US, fig.height=5, fig.width=16, warning=FALSE}
#read in the data
data("slf_usa_df")
data("slftoh_usa_df")
data("toh_usa_df")
...
```{r visualize models at World, fig.height=5, fig.width=16, warning=FALSE}
#read in the data
data("slf_df")
data("slftoh_df")
data("toh_df")
...
#plot just the states and suitability first
m1 <- ggplot() +
  geom_raster(data = toh_df, aes(x = x, y = y, fill = rescale(value)), alpha=0.9, show.legend = F) +
  scale_fill_gradientn(limits= c(0,1), name = "Suitability", colors = rev(c("#e31a1c","#fd8d3c", "#fecc5c", "#FFFFFF"))) +
  ...
  ggtitle(label = "m1")
m2 <- ggplot() +
  geom_raster(data = slf_df, aes(x = x, y = y, fill = rescale(value)), alpha=0.9, show.legend = F) +
  scale_fill_gradientn(limits= c(0,1), name = "Suitability", colors = rev(c("#e31a1c","#fd8d3c", "#fecc5c", "#FFFFFF"))) +
  geom_polygon(data = map_data('world'), aes(x = long, y = lat, group = group), fill = NA, color = "black", lwd = 0.15) +
  theme_bw() +
  labs(x = "", y = "") +
  ...
  ggtitle(label = "m2")
s3 <- ggplot() +
  geom_raster(data = slftoh_df, aes(x = x, y = y, fill = rescale(value)), alpha=0.9, show.legend = T) +
  scale_fill_gradientn(limits= c(0,1), name = "Suitability", colors = rev(c("#e31a1c","#fd8d3c", "#fecc5c", "#FFFFFF"))) +
  geom_polygon(data = map_data('world'), aes(x = long, y = lat, group = group), fill = NA, color = "black", lwd = 0.15) +
  theme_bw() +
  labs(x = "", y = "") +
