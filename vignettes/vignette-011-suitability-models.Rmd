...
#change weird ATC prefixes
output.files <- gsub(pattern = "2008_accessibility_to_cities_50k", replacement = "global_atc1", x = output.files) #2008 version of ATC data
output.files <- gsub(pattern = "2015_accessibility_to_cities_v1.0", replacement = "global_atc2", x = output.files) #2015 version of ATC data, "improved"
...
write_csv(x = env.corr, path = file.path(here(),"data-raw/env_cor_v1.1_downsampled.csv"), col_names = TRUE)
...
env.cor <- read.csv(file = file.path(here::here(),"data-raw/env_cor_v1.1_downsampled.csv"), row.names = 1)
...
After fitting individual models, we fit these 6 covariates combined to SLF and TOH presences, but **BIO02** and **ELEV** explained <1.0% of the variation, so we removed them and refit the remaining covariates under the settings above, except that the number of `Replicates = 10` for **TOH**, rather than `Replicates = 5`. The resultant two models are:
  
  1. *m1*---a multivariate SDM of TOH
  2. *m2*---a multivariate SDM of SLF
  
A third model was created from *m1* suitability and **SLF** presence records, based on evidence supporting the strong affinity of SLF for TOH as a preferred host (@parra_usda-aphis-ppq-cphst_2018, @urban_perspective:_2019):

  3. *s3*---a single variate SDM of SLF
...
The three models performed similarly (notably without much difference between the versions of *m1* and *m2* that included **BIO02** and **ELEV**, see above for more detail), albeit with small differences across model performance metrics. Thus, we created a consensus output map by averaging the three models as an ensemble.
...
enm_data_slf <- raster(file.path(mypath, "maxent_models/02_15_19_maxent_slf+atc-bio02/lycorma_delicatula_avg.asc"))
...
countries_extracts_ensemble2 <- extract_enm2(enm = enm_ensemble, geoshape = world, id0 = "NAME_0", id = as.character(world$NAME_0), multipar = TRUE, ncores = 5)
#write out
