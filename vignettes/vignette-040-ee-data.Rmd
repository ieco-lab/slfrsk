...
link-citations: yes
resource_files:
  - slfrsk_ee_home_label.png
  - slfrsk_ee_layers_panel.png
  - docs/articles/slfrsk_ee_home_label.png
  - docs/articles/slfrsk_ee_layers_panel.png
---
...
Google Earth Engine App Companion
...
The `slrsk` EE App is an interactive map that allows for highly customizable visualization. When loaded, it will display a map of the earth with several different data layers already enabled (see image below for example).

<center>

![Landing page and labeled visualization tools for `slfrsk` EE App.](/Volumes/GoogleDrive/My Drive/spotted_lanternfly_ieco_projects/code/FINAL_CODE/R package/nah_work/slfrsk/docs/articles/slfrsk_ee_home_label.png)
...
![Default view of Layers Panel for `slfrsk` EE App.](/Volumes/GoogleDrive/My Drive/spotted_lanternfly_ieco_projects/code/FINAL_CODE/R package/nah_work/slfrsk/docs/articles/slfrsk_ee_layers_panel.png)
...
3. **vineyards US**: areas reported to be cultivated as vineyards in the US according to @usda_national_agricultural_statistics_service_nass_cropscape_2019 are  depicted by **<span style="color:#800080;">purple pixels</span>**
...
5. **transport potential**: the potential for transport of SLF as determined by the metric tonnage of trade with US states that are considered to have established SLF populations (scaled $[0,10]$). This layer is a border outline for countries and US states.
...
6. **invasion risk**: the risk of shock to the wine market as determined by the relationship between predicted values of average annual grape production based on SLF transport and establishment potential and average annual wine market size (scaled $[1,10]$). This scaling ranges from a completely negative relationship at 1 to a completely positive relationship at 10 (thus explaining the change from the $[0,10]$ scale seen for other layers) This layer is a shaded fill for *only* `countries` that **do not already have established SLF populations**.

```{r risk key, echo=FALSE, fig.width=5, fig.height=1, fig.align='center'}
tibble(x= seq(from = 0, to = 10, by = 1), colors = rev(c('#a50026','#d73027','#f46d43','#fdae61','#fee090','#ffffbf','#e0f3f8','#abd9e9','#74add1','#4575b4','#313695'))) %>%
ggplot(data = .) +
  geom_tile(aes(x = x, y = 1, fill = colors), show.legend = F) +
scale_fill_manual(
...
This vignette is a companion to the interactive `slfrsk` interactive `slfrsk` Google Earth Engine (EE) App <https://ieco.users.earthengine.app/view/ieco-slf-riskmap>. It provides a brief description of the various visualizations (including keys and legends) and demonstrates how the data were treated prior to inclusion in the app. Output data from this vignette are saved in the shared GoogleDrive `./slfRiskMapping/data/slfRisk/` directory. 

## Setup

Here we load the packages that allow us to visualize EE color palettes and reformat shapefile data and merge with the summarized potentials data visualized in `vignette-021-quadrant-plots`.
...
```{r add the corr plot data, eval=FALSE}
#states
...
```{r load and merge market_risk data, eval=FALSE}
...
```{r read in the country and state shapefiles, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r clean the shape data, eval=FALSE}
#STATES
#change DC name
...
```{r make shapefiles smaller, eval=FALSE}
...
```{r filter the states to just those in the trade data, eval=FALSE}
#states
states <- states[states@data$NAME_1 %in% unique(states_summary_future_ensemble$geopol_unit),]
states_lo <- states_lo[states_lo@data$NAME_1 %in% unique(states_summary_future_ensemble$geopol_unit),]
#countries
countries <- countries[countries@data$NAME_0 %in% unique(countries_summary_future_ensemble$geopol_unit),]
countries_lo <- countries_lo[countries_lo@data$NAME_0 %in% unique(countries_summary_future_ensemble$geopol_unit),]
```
...
```{r make sure geometries are  clean, warning=FALSE, message=FALSE, eval=FALSE}
...
```{r add in the trade data, eval=FALSE}
#states
states@data <- left_join(states@data, states_summary_future_ensemble, by = c("NAME_1" = "geopol_unit"))
states_lo@data <- left_join(states_lo@data, states_summary_future_ensemble, by = c("NAME_1" = "geopol_unit"))
#countries
countries@data <- left_join(countries@data, countries_summary_future_ensemble, by = c("NAME_0" = "geopol_unit"))
countries_lo@data <- left_join(countries_lo@data, countries_summary_future_ensemble, by = c("NAME_0" = "geopol_unit"))
...
```{r merge to a single shapefile, message=FALSE, eval=FALSE}
...
```{r maybe test try plotting trade data, warning=FALSE, eval=FALSE}
...
states_gg <- left_join(states_gg, states_summary_future_ensemble, by = c("id" = "geopol_unit"))
...
```{r try to test plot, fig.height=5, fig.width=7, eval=FALSE}
