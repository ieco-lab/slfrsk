...
Build and Save Risk Correlation Quadrant Plots
...
Secondarily, we confirm the particular partitions of data we intend to visualize (defaults match the published figures). Here is where one could change the version of data for the correlation figure via the following: 
  ...
  6. rem_usa: Remove the "United States" from the `countries` plot?
      - default is `TRUE`
  7. label.countries.est: Label the countries that *Lycorma* has previously become established (Japan and South Korea) in the `countries` plot?
      - default is `TRUE`
  8. label.states.est: Label the U.S. states that *Lycorma* has previously become established according to `present` or `future` scenarios in the `states` plot?
      - default is `TRUE`
...
The code below loads the summary data from `vignette-010-tidy-data` based on the logical status of `present_transport` above to keep the environment clean. Both `states` and `countries` data are loaded.

```{r read in summary data}
#decide which data to call
...
**NOTE:** Below is code that can be used to make a version of the `country` data that removes all countries that have a no `trade` with infected states (*we leave the zeroes in for the current figures*):
...
```{r write out the summary data together, eval=FALSE}
states_summary_future_pred %>% 
  mutate(category = "state") %>%
  bind_rows(., countries_summary_future_pred %>% mutate(category = "country")) %>%
