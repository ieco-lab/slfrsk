...
nudge_us.infected_present.x <- c(0,0,0)  #PRESENT: adding DE, MD, NJ  
nudge_us.infected_present.y <- c(0,0,0)
nudge_us.infected_future.x <- c(0,0,0,0,0)  #FUTURE: adding DE, MD, NJ, OH, WV
nudge_us.infected_future.y <- c(0,0,0,0,0)
#lx <- 0.22
#ly <- 0.25
#hy <- 0.77
#hx <- 0.78
...
if(present_transport == FALSE){
  if(impact_prod == "wine"){
    ...
    if(statez) { #STATES FUTURE GRAPES
      ...
      nudge.y <- c(-.10, .10, .10, .10,-.05, .10,-.05,-.07,-.12, .10)   #production: CA, WA, NY, PA, MI, OR, TX, VA, NC, MO
      if(label.us.infected){
        nudge.x <- c(nudge.x, nudge_us.infected_future.x)
        nudge.y <- c(nudge.y, nudge_us.infected_future.y)
      }
  ...
  if(impact_prod == "wine"){
    ...
    if(statez) { #STATES PRESENT GRAPES
      nudge.y <- c(-0.05, 0.095, -0.10, -0.12, -0.05, 0.085, -0.02, 0.12, -0.10, 0.13)  #CA, WA, NY, PA, MI, OR, TX, VA, NC, MO  
...
if(present_transport){
    ...
    pdf(file.path(here::here(),"vignettes", paste0("countries_combined_future_max_",impact_prod,".pdf")),width = 7.5, height = 7)
      ...
      grid.arrange(countries_grob, legend_state, nrow=1, ncol = 2, widths = c(4,2), heights = 1)
    dev.off()
