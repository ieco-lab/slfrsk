...
library(scales) #make custom tranform object for plotly corplots
```
...
The following plotting parameters are set to produce the published figures but could be changed for plot customization:
  ...
  2. label.states.est: Label the U.S. states that *Lycorma* has previously become established according to `present` or `future` scenarios in the `states` plot?
      - default is `TRUE`
      - adds CT, DE, MD, NJ, OH, WV (all others already labeled)
...
Despite the focus of `Vignette-041-market-plot`, we also tag on a quick piece of code that will calculate the correlations for both versions of the `impact_prod` for the `present` case. We remove `countries` with established *Lycorma* populations.
...
`Plotly`-based visualization of each alignment plot makes it easier to see individual points.

```{r make custom transform object with scales::trans_new, echo=FALSE}
#make custom transform object with scales::trans_new
logp1_rnd <- trans_new(
  name = "logp1_rnd",
  transform = function(x) round(x = log10(x + 1), digits = 2),
  inverse = function(x) round(x = (exp(x) - 1), digits = 2),
  breaks = log_breaks()
)
```

```{r plotly version of states scatter, message=FALSE, warning=FALSE, fig.height= 8, fig.width=8, echo=FALSE}
#plotly version of states scatter
states_ggplot <- states_summary_present %>%
  mutate_if(.predicate = is.numeric, .funs = ~round(x = ., digits = 2)) %>%
  mutate(`establishment potential` = grand_mean_max_scaled, `transport potential` = avg_infected_mass_scaled, avg_wine = signif(avg_wine, digits = 3)) %>%
  ggplot(data = ., aes(y = `transport potential`, x = `establishment potential`, size = avg_wine, fill = grapes, color = status)) +
      geom_hline(yintercept = q_intercepts$establishment, linetype = "dashed") +
      geom_vline(xintercept = q_intercepts$transport, linetype = "dashed") +
      geom_point(aes(text = ID), shape = 21, stroke = 1.3, alpha = 0.75, show.legend = F) +
      scale_fill_manual(
        values = c("no" = "#ffffff", "yes" = "#C77CFF"),
      ...
      scale_size_continuous(name = "Grape Production",
                            #trans = "log10",
                            trans = logp1_rnd, #custom rounding added to log10(x+1)
                            range = c(1.5, 6),
                            breaks = c(1.5,2,6),
                            labels = c("low", "moderate", "high")
      ) +
      ...
      scale_x_continuous(breaks = c(0,.5,1),labels = c("low", "moderate", "high"), lim = c(0,1.1)) +
      scale_y_continuous(breaks = c(0,.5,1),labels = c("low", "moderate", "high"), lim = c(0,1.1))

ggplotly(states_ggplot, dynamicTicks = F, tooltip = c("ID", "establishment potential", "transport potential", "avg_wine", "grapes", "status")) %>%
  layout(xaxis = 
           list(title = list(font = list(size = 11)), showspikes = T),
         yaxis = 
           list(title = list(font = list(size = 11)), showspikes = T),
         title = 
           list(text = "U.S. States Invasion Potential Alignment",
                font = list(size = 12),
                x = 0.1,
                y = 0.99),
         showlegend = F
...
```{r plotly version of countries scatter, message=FALSE, warning=FALSE, fig.height= 8, fig.width=8, echo=FALSE}
#plotly version of countries scatter
countries_ggplot <- countries_summary_present %>%
  mutate_if(.predicate = is.numeric, .funs = ~round(x = ., digits = 2)) %>%
  mutate(`establishment potential` = grand_mean_max_scaled, `transport potential` = avg_infected_mass_scaled, avg_wine = signif(avg_wine, digits = 3)) %>%
  ggplot(data = ., aes(y = `transport potential`, x = `establishment potential`, size = avg_wine, fill = grapes, color = status)) +
      geom_hline(yintercept = q_intercepts$establishment, linetype = "dashed") +
...
ggplotly(countries_ggplot, dynamicTicks = F, tooltip = c("ID", "establishment potential", "transport potential", "avg_wine", "grapes", "status")) %>%
  layout(xaxis = 
           list(title = list(font = list(size = 11)), showspikes = T),
         yaxis = 
           list(title = list(font = list(size = 11)), showspikes = T),
         title = 
           list(text = "Countries Invasion Potential Alignment",
                font = list(size = 12),
                x = 0.1,
                y = 0.99),
         showlegend = F
...
We can also save the static plots to `/vignettes` as a `.pdf`. Note that the saving chunks can be run by setting the top `if` from `FALSE` to `TRUE`!

```{r optional write to pdf step, message=FALSE, eval=TRUE, fig.height= 12, fig.width=10}
if(FALSE){
...
```{r combined plot with separate legend pdf, eval=TRUE}
if(FALSE){
