...
We now load the summary data from `vignette-010-tidy-data` based on the logical status of `present_transport`. Both `states` and `countries` data are loaded.
...
Even though a column in the read data does include the $log_{10}(+1)$ transformation of **transport** potential, the mass of trade with infected states (`log10_avg_infected_mass`), the raw data were $log_{10}(+1)$ transformed and then rescaled (see `one_zero` above) as `avg_infected_mass_scaled`. The **establishment** potential is already scaled the same [0,1], but we create a new column of its values with teh same `_scaled` suffix. If `one_zero <- FALSE`, then **transport** potential is just $log_{10}(+1)$ transformed instead.
