...
Now plot the states
```{r the US map, fig.height= 6.5, fig.width=7}
...
countries_trade_final <- left_join(countries_summary_present, countries_centers, by = "geopol_unit") %>%
  ...
  dplyr::select(origin, everything())
    ...
    #now merge the coords for destinations (ONLY WINE PRODUCING countries)
countries_trade_final <- left_join(countries_summary_future, countries_centers, by = "geopol_unit") %>%
  filter(!is.na(x), !is.na(y))
...
1. make arrows plot to the capitals of countries for coordinates rather than the centroids (change the x and y values in `countries_trade_final`)
2. add in the legend with corrections from JEB
  - Top 10 importers = high transport potential
  - viticultural area = high impact potential
  - suitablility = establishment potential



```{r plot map of countries, warning=FALSE, fig.height= 6.5, fig.width=7}
