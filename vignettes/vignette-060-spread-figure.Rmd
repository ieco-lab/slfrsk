...
vignette: >
  %\VignetteIndexEntry{vignette-xxx-misc}
...
library(sp) #convert sf shapefiles to sp
library(magrittr) #fast pipe assign
library(lubridate) #extract years
```

now try to read in the seba SLF records data
...
generate the shapefile for the counties (~3mb download)
```{r counties shapefile, message=FALSE}
counties <- counties(cb = T, resolution = "5m")

#alternate version of counties from GADM
counties2 <- st_read(dsn = "/Volumes/GoogleDrive/Shared drives/slfData/data/slfRisk/geo_shapefiles/gadm36_USA_shp/", layer = "gadm36_USA_2") %>%
  rename(GEOID = GID_2, NAME = NAME_2)
```

match coordinates to counties
```{r}
#adapted from SDB code in lycordata
factor_order <- c("Unpopulated", "Low", "Medium", "High")

tinyslf %<>% 
  add_column(row_ID = 1:nrow(.)) %>% 
  mutate(slf_density = factor(slf_density, levels = factor_order))
...
now to do the seba check of the data with some manual entry  data
```{r}

# we can use the FIPS code to merge to the main data
# now let's extract the status and the year of record/establishment
# Let's check the promising "Status" columns
table(man_county_data$Status)
# we can use this column to define establishment vs record
...
ggplot() +
  geom_sf(data = by_county %>% filter(!GEOID %in% focal_counties), fill = "#FFFFFF") +
  geom_sf(data = by_county %>% filter(GEOID %in% focal_counties), aes(color = FirstRecord, fill = YearOfEstablishment), show.legend = F) +
...
ggplot() +
  geom_polygon(data = broom::tidy(as_Spatial(by_county), region = "NAME") %>% left_join(., as_Spatial(by_county)@data, by = c("id" = "NAME")), aes(x=long,y=lat, group = group, color = FirstRecord, fill = YearOfEstablishment)) +
#    geom_polygon(data = broom::tidy(as_Spatial(by_county), region = "NAME") %>% left_join(., as_Spatial(by_county)@data, by = c("id" = "NAME")), aes(x=long,y=lat, group = group, color = NA, fill = YearOfEstablishment)) +
  coord_quickmap(xlim = c(-124.7628, -66.94889), ylim = c(24.52042, 49.3833), expand = FALSE) +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
